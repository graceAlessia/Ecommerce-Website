...
if (isset($_GET['order_id'])) {
    ...
    $stmt->execute();
    $order = $stmt->get_result()->fetch_all(MYSQLI_ASSOC); // Fetching as associative array
} else if (isset($_POST['edit_order'])) {

    $order_status =  $_POST['order_status'];
    $order_id = $_POST['order_id'];

    $stmt = $conn->prepare("UPDATE orders SET order_status=? WHERE order_id=?"); // Removed extra comma
    $stmt->bind_param('si', $order_status, $order_id);
    if ($stmt->execute()) {
        header('Location: dashboard.php?order_success=Order has been edited successfully!');
        exit; // Stop further script execution
    } else {
        header('Location: dashboard.php?order_fail=There is an error. Please try again.');
        exit; // Stop further script execution
    }
} else {
                                ...
                                <div class="form-group my-3">
                                    ...
                                    <select class="form-select" required name="order_status">
                                        ...
                                        <option value="delivered" <?php if ($r['order_status'] == 'delivered') {
                                                                        echo "selected";
                                                                    } ?>>Delivered</option> <!-- Fixed option value -->
                                    </select>
                                </div>
